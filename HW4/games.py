...
class Hexapawn(Game):
    ...
    def __init__(self, h=3, v=3):
        ...
        moves = self._legal_moves(board, 'X')
    ...
    def result(self, state, move):
        ...
        if utility != 0:
            ...
            next_moves = self._legal_moves(board, next_player)

            if len(next_moves) == 0:
                utility = 1 if player == 'X' else -1
    ...
    def _legal_moves(self, board, player):
        ...
        for (x, y), piece in board.items():
            ...
            if player == 'X':
                ...
                if forward_x >= 1 and (forward_x, y) not in board:
                    moves.append(((x, y), (forward_x, y)))

                
                for dy in (-1, 1):
                    new_y = y + dy

                    if (forward_x, new_y) in board:
                        if board[(forward_x, new_y)] == 'O':
                            moves.append(((x, y), (forward_x, new_y)))

            else:
                forward_x = x + 1

                
                if forward_x <= self.h and (forward_x, y) not in board:
                    moves.append(((x, y), (forward_x, y)))
